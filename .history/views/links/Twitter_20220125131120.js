  ...
  const getData = async (twitterURL) => {
    ...
    try {
      const response = await fetch(
        `https://api.twitter.com/2/tweets/${after_}?expansions=attachments.media_keys,author_id&tweet.fields=attachments,created_at&user.fields=profile_image_url,url&media.fields=duration_ms,height,width,preview_image_url,type,url,alt_text`,
        {
          method: "GET",
          headers: {
            Authorization:
    ...
    <View style={styles.container}>
      ...
      {data.data.text && (
        <View style={styles.content}>
          <Copy>{data.data.text}</Copy>
          <Text>{timeAgo.format(new Date(data.data.created_at), "round-minute")}</Text>
        </View>
      )}
      {data && data.includes && (
        <Image
