...
const ImageStack = (props) => {
  ...
  const renderTwo = (mediaArray) => (
    ...
    >
      <Container style={{ flex: 0.5 }}>{renderImage(mediaArray[0])}</Container>
      <Split />
      <Container style={{ flex: 0.5 }}>{renderImage(mediaArray[1])}</Container>
    </Container>
  );
  ...
  const render = () => {
    ...
    switch (props.media.length) {
      ...
      case 2:
        return renderTwo(props.media.slice(2));
      default:
        return renderImage(props.media[0]);
    }
  };
