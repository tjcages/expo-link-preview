  ...
  const renderLink = () => {
    return props.linkData.image ? (
      ...
      <WebView
        ...
        source={{
          body: `<html><head><meta name='viewport' content='width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=no'><script>
        </script></head><body><div id='wrapper'></div>
        </body></html>`,
        }}
        scrollEnabled={false}
        disabled={true}
        ...
        originWhitelist={["*"]}
        onMessage={() => {}}
        style={{backgroundColor: "red", margin: 16,}}
      />
    );
  };
  ...
  return (
    <TouchableOpacity
      onPress={() => {
        let loadScript = `
        function myFunction() {
          document.getElementById("wrapper").style = "background-color: #ff0000;";
        }
        `;
